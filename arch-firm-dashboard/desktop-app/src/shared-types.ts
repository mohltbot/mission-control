...
export interface ActivityLog {
  id: string;
  employeeId: string;
  type: 'task_started' | 'task_completed' | 'timer_started' | 'timer_stopped' | 'timer_paused' | 'timer_resumed' | 'idle_detected' | 'screenshot_captured' | 'app_opened' | 'app_closed';
  timestamp: Date;
...
export interface ClientToServerEvents {
  'auth:login': (data: { employeeId: string; token: string }) => void;
  'auth:logout': () => void;
  'time:start': (data: { taskId: string; projectId: string }) => void;
  'time:stop': (data: { entryId: string; notes?: string }) => void;
  'activity:log': (data: Omit<ActivityLog, 'id' | 'timestamp'>) => void;
  'idle:detected': (data: { duration: number }) => void;
  'idle:returned': (data: { idleDuration: number }) => void;
  'heartbeat': () => void;
}
