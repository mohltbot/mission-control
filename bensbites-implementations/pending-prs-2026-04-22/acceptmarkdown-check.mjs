...
async function checkMarkdownCompanion(baseUrl) {
  ...
  const candidates = [
    // Common agent-friendly conventions
...
async function checkRobots(baseUrl) {
  ...
  try {
    ...
    // Naive check: if any of our agent UAs are mentioned with a Disallow /, flag it.
    const blocked = [];
...
function score(r) {
  // 0..5. One point each: reachable, has semantic html, has structured data,
  // has an md companion OR llms.txt, robots doesn't block agents.
  let s = 0;
